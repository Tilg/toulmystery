...
public class DeviceHandler : MonoBehaviour {
	...
	void Awake(){
		...
		if (currentDPI == Constants.DEFAULT_DPI && currentWidth == Constants.DEFAULT_SCREEN_WIDTH && currentHeight == Constants.DEFAULT_SCREEN_HEIGTH ){ // if we are in the default mode or if the device is an ipadMini retina
