...
public class MapInitialisation : MonoBehaviour {
	...
	void Start () {
		...
		GameObject[] listeCheckpoint = GameObject.FindGameObjectsWithTag("Checkpoint");
		
		foreach ( GameObject checkpoint in listeCheckpoint){
			Checkpoint checkpointTmp = (Checkpoint) checkpoint.GetComponent("Checkpoint");
			
			Transpose.placeGameObjectAt(checkpoint,checkpointTmp.latitude,checkpointTmp.longitude);

			Debug.Log(checkpointTmp.id+" lat : "+ checkpointTmp.latitude);	
			Debug.Log(checkpointTmp.id+" long : "+ checkpointTmp.longitude);
		}
	}
